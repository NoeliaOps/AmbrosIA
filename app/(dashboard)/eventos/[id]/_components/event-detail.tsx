          ...
          <div className="rounded-lg border border-border p-4 space-y-1">
            ...
            {event.clients?.phone && (
              <div className="flex items-center gap-2">
                ...
                <a
                  href={`https://wa.me/${event.clients.phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${event.clients.name}, te contactamos de Artesano Banquetes respecto a tu evento *${event.name}* del ${new Date(event.event_date).toLocaleDateString("es-MX", { day: "numeric", month: "long", year: "numeric" })}.`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-[11px] font-sans text-emerald-700 hover:text-emerald-800 transition-colors"
                ...
                {event.clients?.phone && (
                  <a
                    href={`https://wa.me/${event.clients.phone.replace(/\D/g, "")}?text=${encodeURIComponent(
                      ...
                      `Quedamos a tus órdenes. — Artesano Banquetes`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 h-7 rounded-md border border-emerald-200 bg-emerald-50 px-2.5 text-xs font-sans text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300 transition-colors"
                  >
